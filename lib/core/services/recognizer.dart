...
class Recognizer {
  Future loadModel() {
    ...
    return Tflite.loadModel(
      model: "assets/model_ai/mnist.tflite",
      labels: "assets/model_ai/mnist.txt",
    );
  }
  ...
  Future<Uint8List?> previewImage(List<Offset> points) async {
    ...
    final image =
        await picture.toImage(Constants.mnistImageSize, Constants.mnistImageSize);
    var pngBytes = await image.toByteData(format: ImageByteFormat.png);

    return pngBytes?.buffer.asUint8List();
  }

  Future<dynamic> recognize(List<Offset> points) async {
    if (points.isEmpty) return null;
  ...
  Future<dynamic> _predict(Uint8List bytes) {
    return Tflite.runModelOnBinary(binary: bytes);
  }

  Future<dynamic> predict(Uint8List bytes) {
    return Tflite.runModelOnBinary(binary: bytes);
  }
  ...
  Future<Uint8List> _imageToByteListUint8(Picture pic, int size) async {
    ...
    if (imgBytes == null) return Uint8List(0);
